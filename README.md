...
Offline Android app for to-do reminders, diary tracking, sleep tracking, water checks, analysis, and complete JSON backup/restore.

- To-dos have optional notes, weekday selection, reminder time, and an option allowing an image on each daily completion.
- A selected date can be claimed for a to-do; completions remain limited to one per to-do per date and can be edited or deleted.
- Completion images are private app files with attach, replace, remove, preview, export, and import support.
...
- Water checks are stored with amount and exact time and can be edited or deleted.
...
- Export writes all rows and Base64 completion images as JSON to public `Download/TodoDiary`; import validates and transactionally replaces local data after two confirmations.
- Date-based pages use a distinct background only when the selected date is today.
...
2. Select any date and claim the matching completion. The same task cannot be completed twice on one date. Attach an image when that schedule allows it.
...
6. Use Water to add each 250 ml check. Edit or delete a wrong historical entry.
...
9. Use Backup to export or import complete JSON data, including attached images. Data stays offline unless you move or share the file.

Date and time input:

- Date pages have `Previous`, `Select date`, `Today`, and `Next` controls that load the chosen date immediately.
